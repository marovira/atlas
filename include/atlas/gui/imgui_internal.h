...
#    define IM_NEWLINE \
        "\r\n" // Play it nice with Windows users (2018/05 news: Microsoft
               // announced that Notepad will finally display Unix-style
               // carriage returns!)
#else
#    define IM_NEWLINE "\n"
#endif
...
ImStrchrRange(const char* str_begin, const char* str_end, char c);
...
struct ImGuiContext
{
    ...
    int FocusRequestCurrCounterAll;      // Any item being requested for focus,
                                    ...
                                    // stable between the frame pressing TAB and
...
namespace ImGui
{
    ...
    IMGUI_API void
    Shutdown(ImGuiContext* context); // Since 1.60 this is a _private_ function.
    ...
    IMGUI_API bool
    ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb = NULL);
    IMGUI_API bool ItemHoverable(const ImRect& bb, ImGuiID id);
    IMGUI_API bool
    IsClippedEx(const ImRect& bb, ImGuiID id, bool clip_even_when_logged);
    IMGUI_API bool
    ...
    IMGUI_API void
    SetNavIDWithRectRel(ImGuiID id, int nav_layer, const ImRect& rect_rel);
    ...
    IMGUI_API ImGuiColumns* FindOrCreateColumns(ImGuiWindow* window,
                                                ImGuiID id);
    ...
    IMGUI_API void
    RenderColorRectWithAlphaCheckerboard(ImVec2 p_min,
                                         ImVec2 p_max,
                                         ImU32 fill_col,
                                         float grid_step,
                                         ImVec2 grid_off,
    ...
    IMGUI_API void
    RenderMouseCursor(ImDrawList* draw_list,
                      ImVec2 pos,
                      float scale,
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // 2019/06/07: Updating prototypes of some of the internal functions.
    // Leaving those for reference for a short while.
    inline void RenderArrow(ImVec2 pos, ImGuiDir dir, float scale = 1.0f)
    {
        ImGuiWindow* window = GetCurrentWindow();
        RenderArrow(
            window->DrawList, pos, GetColorU32(ImGuiCol_Text), dir, scale);
    }
    ...
    TreeNodeBehaviorIsOpen(ImGuiID id,
                           ImGuiTreeNodeFlags flags =
                               0); // Consume previous SetNextItemOpen() data,
...
extern void
ImGuiTestEngineHook_ItemAdd(ImGuiContext* ctx, const ImRect& bb, ImGuiID id);
