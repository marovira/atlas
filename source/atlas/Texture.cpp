...
namespace atlas
{
    ...
    void Texture::loadTexture(std::string const& filename)
    {
        ...
        gl GL;
        gl::format const format = GL.translate(tex.format());
                    ...
                    case TARGET_CUBE:
                        ...
                        else
                        {
                            glTexSubImage2D(target,
                                ...
                                dimensions.x,
                                (tex.target() == TARGET_1D_ARRAY) ?
                                    layerGL : dimensions.y, format.External,
                    ...
                    case TARGET_CUBE_ARRAY:
                        if (is_compressed(tex.format()))
                        {
                            glCompressedTexSubImage3D(target,
                                ...
                                (tex.target() == TARGET_3D) ? dimensions.z :
                                layerGL, format.Internal,
                                static_cast<GLsizei>(tex.size(level)),
                                tex.data(layer, face, level));
                        }
                        else
                        {
                            glTexSubImage3D(target,
                                ...
                                dimensions.x, dimensions.y,
                                (tex.target() == TARGET_3D) ? dimensions.z :
                                layerGL, format.External, format.Type,
                                tex.data(layer, face, level));
                        }
