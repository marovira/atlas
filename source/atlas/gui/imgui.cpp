...
void ImGui::UpdateMouseWheel()
{
    ...
    const bool scroll_allowed =
        ...
        !(window->Flags & ImGuiWindowFlags_NoMouseInputs);
    if (scroll_allowed &&
        (g.IO.MouseWheel != 0.0f || g.IO.MouseWheelH != 0.0f) && !g.IO.KeyCtrl)
    {
...
bool ImGui::IsMouseClicked(int button, bool repeat)
{
    ...
    if (repeat && t > g.IO.KeyRepeatDelay)
    {
        ...
        // mouse hold.
        int amount =
            CalcTypematicPressedRepeatAmount(t,
                                             t - g.IO.DeltaTime,
                                             g.IO.KeyRepeatDelay,
                         ...
                         ImGuiColumnsFlags flags)
{
    ...
    columns->OffMaxX =
        ImMax(window->WorkRect.Max.x - window->Pos.x, columns->OffMinX + 1.0f);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
                                 ...
                                 const char* label)
        {
                 ...
                 elem_offset += pcmd->ElemCount, pcmd++)
            {
                    ...
                    for (int i = elem_offset;
                        ...
                        vtxs_rect.Add(
                            draw_list->VtxBuffer[idx_buffer ? idx_buffer[i] : i]
                                .pos);
                    clip_rect.Floor();
                    fg_draw_list->AddRect(clip_rect.Min,
                                          clip_rect.Max,
                                          IM_COL32(255, 0, 255, 255));
                    vtxs_rect.Floor();
                    fg_draw_list->AddRect(vtxs_rect.Min,
                                          vtxs_rect.Max,
                                          IM_COL32(255, 255, 0, 255));
                }
                if (!pcmd_node_open)
                ...
                ImGui::Text("ElemCount: %d, ElemCount/3: %d, VtxOffset: +%d, "
                            "IdxOffset: +%d",
                            pcmd->ElemCount,
                            pcmd->ElemCount / 3,
                            pcmd->VtxOffset,
        ...
        static void NodeWindow(ImGuiWindow* window, const char* label)
        {
            ...
            ImGui::BulletText(
                ...
                (window->Active || window->WasActive)
                    ...
                    : -1);
            ImGui::BulletText("Appearing: %d, Hidden: %d (CanSkip %d Cannot "
                              "%d), SkipItems: %d",
                              window->Appearing,
                              window->Hidden,
                              window->HiddenFramesCanSkipItems,
    ...
    if (ImGui::TreeNode("Tools"))
    {
        ...
        const char* rects_names[RT_Count] = {"OuterRect",
                                             "OuterRectClipped",
                                             "InnerRect",
                                             "InnerClipRect",
                                             "WorkRect",
        ...
        show_windows_rects |= ImGui::Combo(
            "##rects_type", &show_windows_rect_type, rects_names, RT_Count);
