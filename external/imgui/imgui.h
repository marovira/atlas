...
namespace ImGui
{
    ...
    EndFrame(); // ends the Dear ImGui frame. automatically called by Render(),
                // you likely don't need to call that yourself directly. If you
                // don't need to render data (skipping rendering) you may call
                // EndFrame() but you'll have wasted CPU already! If you don't
                // need to render, better to not create any imgui windows and
                // not call NewFrame() at all!
    IMGUI_API void
    Render(); // ends the Dear ImGui frame, finalize the draw data. You can get
              // call GetDrawData() to obtain it and run your rendering
              // function. (Obsolete: this used to call io.RenderDrawListsFn().
              // Nowadays, we allow and prefer calling your render function
              // yourself.)
    ...
    GetVersion(); // get the compiled version string e.g. "1.23" (essentially
    ...
    IMGUI_API bool IsWindowHovered(
        ImGuiHoveredFlags flags =
            0); // is current window hovered (and typically: not blocked by a
                ...
                // your app, you should use the 'io.WantCaptureMouse' boolean
                // for that! Please read the FAQ!
    ...
    IMGUI_API void SetNextWindowSizeConstraints(
        ...
        void* custom_callback_data =
            NULL); // set next window size limits. use -1,-1 on either X/Y axis
                   // to preserve the current size. Sizes will be rounded down.
                   // Use callback to apply non-trivial programmatic
                   // constraints.
    ...
    IMGUI_API void PushItemWidth(
        float item_width); // set width of items for common large "item+label"
                           ...
                           // pixels to the right of window (so -1.0f always
    ...
    IMGUI_API void
    SetCursorPos(const ImVec2& local_pos); //    are using the main, absolute
                                           //    coordinate system.
    IMGUI_API void
    ...
    IMGUI_API bool InvisibleButton(
        const char* str_id,
        const ImVec2&
            size); // button behavior without the visuals, frequently useful to
    ...
    IMGUI_API void
    Bullet(); // draw a small circle and keep the cursor on the same line.
    ...
    IMGUI_API bool ListBoxHeader(const char* label,
                                 ...
                                 int height_in_items = -1); // "
    IMGUI_API void
    ListBoxFooter(); // terminate the scrolling region. only call
    ...
    BeginPopup(const char* str_id,
               ImGuiWindowFlags flags =
                   0); // return true if the popup is open, and you can start
                       // outputting to it. only call EndPopup() if BeginPopup()
                       // returns true!
    IMGUI_API bool BeginPopupContextItem(
        const char* str_id = NULL,
        int mouse_button =
            1); // helper to open and begin popup when clicked on last item. if
                // you can pass a NULL str_id only if the previous item had an
                // id. If you want to use that on a non-interactive item such as
    ...
    BeginPopupModal(const char* name,
                    bool* p_open = NULL,
                    ImGuiWindowFlags flags =
                        0); // modal dialog (regular window with title bar,
                            // block interactions behind the modal window, can't
    ...
    IMGUI_API bool OpenPopupOnItemClick(
        ...
        int mouse_button = 1); // helper to open popup when clicked on last item
                               ...
                               // _released_ event to be consistent with popup
    ...
    IMGUI_API void
    CloseCurrentPopup(); // close the popup we have begin-ed into. clicking on a
                         // MenuItem or Selectable automatically close the
                         // current popup.
    ...
    BeginChildFrame(ImGuiID id,
                    const ImVec2& size,
                    ImGuiWindowFlags flags =
                        0); // helper to create a child window / scrolling
    ...
    // Inputs Utilities
    IMGUI_API int
    GetKeyIndex(ImGuiKey imgui_key); // map ImGuiKey_* values into user's key
                                     // index. == io.KeyMap[key]
    ...
    IMGUI_API void CaptureKeyboardFromApp(
        bool want_capture_keyboard_value =
            true); // attention: misleading name! manually override
                   ...
                   // capture keyboard when your widget is being hovered. This
                   // is equivalent to setting "io.WantCaptureKeyboard =
    ...
    IMGUI_API void CaptureMouseFromApp(
        bool want_capture_mouse_value =
            true); // attention: misleading name! manually override
                   ...
                   // left for your application to handle). This is equivalent
                   // to setting "io.WantCaptureMouse =
    ...
    IMGUI_API void SaveIniSettingsToDisk(
        const char*
            ini_filename); // this is automatically called (if io.IniFilename is
                           // not empty) a few seconds after any modification
                           // that should be reflected in the .ini file (and
    ...
    // call SetCurrentContext() + SetAllocatorFunctions() again because we use
...
struct ImDrawList
{
    ...
    IMGUI_API void AddImageQuad(ImTextureID user_texture_id,
                                ...
                                ImU32 col          = IM_COL32_WHITE);
    IMGUI_API void
    AddImageRounded(ImTextureID user_texture_id,
                    const ImVec2& a,
                    const ImVec2& b,
                    const ImVec2& uv_a,
                    const ImVec2& uv_b,
...
struct ImFontAtlas
{
    ...
    IMGUI_API int
    AddCustomRectFontGlyph(ImFont* font,
                           ImWchar id,
                           int width,
                           int height,
                           float advance_x,
